...
class NewViewController: UITableViewController, DataTransferProtocol, UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    func createTestNews() {
        let testNews1 = News(id: 1, name: nameLabel.text!, surname: surnameLabel.text!, date: newsTestDateArray[0], text: "rrr", image: newsTestImageArray[0], numberOfLikes: newsTestLikesArray[0], numberOfComments: newsTestCommentsArray[0], numberOfReposts: newsTestRepostsArray[0], avatar: user.avatar )
        ...
        newsManager.syncSave(with: testNews1)
        guard let currectNews: [News] = newsManager.syncGetAll() else {return}
        news = currectNews
    }
    ...
    @objc private func refresh() {
        if let correctNews: [News] = newsManager.syncGetAll() {
            news = correctNews
        } 
